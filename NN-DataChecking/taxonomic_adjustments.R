...
## Updated 25 May 2020 by E. Batzer
...
Taxonomic.Adjustments <- function(datafile = datafile) {
  ...
  # Create new column with old name
  data1$old_taxon <- data1$Taxon
  ...
  temp$Taxon[temp$Taxon %in% c("RHYNCHARRHENA LINEARIS")] <- "VINCETOXICUM LINEARE"
  ...
  temp <- ddply(data1, .(site_code, block, plot, subplot, trt, year, year_trt, Taxon, old_taxon), summarize, max_cover = sum(max_cover))
