...
write.csv(x = bind_rows(storagelist),
  "../../Data/tradeoffs_RRPP_Pvals_Summary.csv")
...
p1 = dot_full %>% # filter(trt_N < .05 & trt_P < .05) %>%
  ...
  #                data = dot_full %>% filter(!(trt_N < .05 & trt_P < .05))) +
  geom_histogram(aes(y = ..count..), color = "black", alpha = .5, position = "identity", bins = 25,
                 data = dot_full, fill = wes_palette("Darjeeling1", 3)[1]) +
  xlim(-1, 1) +
  ggtitle("N - P Correlation") +
  guides(fill = FALSE) + 
  geom_vline(aes(xintercept = mean(NP)), size = 1.5)
...
p2 = dot_full %>% # filter(trt_N < .05 & trt_K < .05) %>%
  ...
  #                data = dot_full %>% filter(!(trt_K < .05 & trt_N < .05))) +  
  geom_histogram(aes(y = ..count..), color = "black", alpha = .5, position = "identity", bins = 25,
                 data = dot_full, fill = wes_palette("Darjeeling1", 3)[2]) +
  xlim(-1, 1) +
  ggtitle("N - K Correlation")+
  guides(fill = FALSE) +
  geom_vline(aes(xintercept = mean(NK)), size = 1.5)
  #scale_y_continuous(limits = c(-4,4), breaks = seq(-6, 6, by = 2), labels = abs( seq(-6, 6, by = 2)))
...
p3 = dot_full %>% # filter(trt_K < .05 & trt_P < .05) %>%
  ...
  #                data = dot_full %>% filter(!(trt_K < .05 & trt_P < .05))) +
    geom_histogram(aes(y = ..count..), color = "black", alpha = .5, position = "identity", bins = 25,
                 data = dot_full, fill = wes_palette("Darjeeling1", 3)[3]) +
  xlim(-1, 1) +
  ggtitle("P - K Correlation")+
  guides(fill = FALSE) +
  geom_vline(aes(xintercept = mean(PK)), size = 1.5)
  # scale_y_continuous(limits = c(-4, 4), breaks = seq(-6, 6, by = 2), labels = abs( seq(-6, 6, by = 2)))
...
```{r}
change_plot <- dot_full %>%
  select(site, N_change, P_change, K_change) %>%
  pivot_longer(c("N_change", "P_change", "K_change"),
               names_to = "direction",
               values_to = "covchange") %>%
  mutate(direction = gsub("_change", "", direction)) %>%
  ggplot(aes(x = factor(direction, levels = c("N", "P", "K")),
             y = covchange)) +
  geom_boxplot(aes(fill = direction), 
               alpha = .5,
               width = .5) +
  ylab("Net Euclidean Change / Log(Year + 1)") +
  xlab("Nutrient Addition Treatment") +
  theme_bw() +
  guides(fill = FALSE) +
  scale_fill_manual(values = wes_palette("Darjeeling1", 3)[c(3, 1, 2)]) +
  ylim(0, 5)

ggsave(plot = change_plot,
      filename = "../../Figures/change_plot.tiff",
       height = 6,
       width = 3)
```
