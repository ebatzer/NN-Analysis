...
# Plotting bivariate distributions of correlation relative to other factors

```{r}
site.summary <- site.attr %>%
  group_by(site_code) %>%
  filter(year_trt == 0) %>%
  summarise(total_richness = unique(site_richness),
            mean_precip = mean(MAP_v2),
            mean_pet = mean(PET),
            pretrt_simpson = mean(inverse_simpson),
            pretrt_alpha = mean(richness_vegan),
            pretrt_beta = mean(plot_beta))


sitecor.summary <- dot_full %>%
  rename("site_code" = "site") %>%
  select(site_code, NP, NK, PK) %>%
  left_join(site.summary, by = "site_code") %>%
  gather(value = "corval",
         key = "cortype",
         NP, NK, PK)

newfunc <- function(selected_var){
  xvar = ensym(selected_var)
