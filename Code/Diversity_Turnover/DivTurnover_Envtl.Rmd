...
# Log transformation of summed sdiances seems approximately normal
# Could summarize euclidean distance within each plot and log-transform?
hist(log(soil.stdized$sd_K + soil.stdized$sd_K + soil.stdized$sd_P))
...
climate_summary = climate %>%
  filter(year < 2019 & 
           ppt > 0 &
           pet > 0) %>%
  mutate(ppt = calc_zscores(log(ppt)),
         pet = calc_zscores(log(pet))) %>%
  group_by(site_code) %>%
