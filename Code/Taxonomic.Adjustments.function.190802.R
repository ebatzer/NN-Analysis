...
Taxonomic.Adjustments <- function(datafile = datafile) {
  ...
  Site = "konz.us"
  temp <- data1[data1$site_code == Site,]
  temp$Taxon[temp$Taxon %in% c("SOLIDAGO CANADENSIS")] <- "SOLIDAGO MISSOURIENSIS"
  temp$Taxon[temp$Taxon %in% c("EUPHORBIA MARGINATA", "EUPHORBIA SERPENS", "EUPHORBIA SPATHULATA")] <- "EUPHORBIA NUTANS"
  temp$Taxon[temp$Taxon %in% c("BAPTISIA AUSTRALIS")] <- "BAPTISIA BRACTEATA"
