...
divstats <- read.csv("C:/Users/ebatzer/Dropbox/NutNet Data/plot_diversity_stats2-04-Dec-2017.csv")
trtlabels <- read.csv("C:/Users/ebatzer/Dropbox/NutNet Data/all-plot-descriptions-1-December-2017.csv")
...
mod_1 <- lmer(alpha.div1 ~ trt + year_trt + trt:year_trt + (1|site_code) + (1|site_code:block),
              data = alpha.byplot[alpha.byplot$trt %in% trts,])
summary(mod_1)
...
tempvar.dat$max_cover <- sqrt(tempvar.dat$max_cover)
...
calcRateChange_bray <- function(comdf, comvar, abundvar, timevar){
  ...
  vd <- vegdist(commat[,-1], method = "bray")
...
m1 <- lmer(distance ~ interval + interval:trt + (1 | site_code) + (1|site_code : block),
     data = temp_ratechange_bray[temp_ratechange_bray$trt %in% trts,],
     REML = FALSE)
